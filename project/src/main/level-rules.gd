...
# generic stuff to eventually pull into a parent script
export (NodePath) var level_cards_path: NodePath

export (int, 8) var difficulty: int = 0 # 0 == very easy, 8 == very hard

var _cell_pos_to_revealed_letter := {}

onready var _level_cards: LevelCards = get_node(level_cards_path)
...
func _add_word(word: String, word_cell_pos: Vector2) -> void:
	...
	for i in range(0, card_word.length()):
		...
		var card := _level_cards.create_card()
		...
		match(letter):
			...
			_:
				...
				if letter == letter.to_upper():
					card.show_front()
		_level_cards.add_card(card, letter_cell_pos)
...
func _show_word(clicked_cell_pos: Vector2) -> void:
	for cell_pos in _cell_pos_to_revealed_letter:
		...
		if cell_pos == clicked_cell_pos:
			...
			continue
		var card := _level_cards.get_card(cell_pos)
